package com.frank.apigateway.filter;
...
import static com.frank.apicommon.enums.UserStatusEnum.BAN;
import static com.frank.apicommon.utils.NetUtil.getIp;
import static com.frank.apigateway.filter.CacheBodyGatewayFilter.CACHE_REQUEST_BODY_OBJECT_KEY;
import static com.frank.apisdk.utils.SignUtil.getSign;
...
public class GatewayGlobalFilter implements GlobalFilter, Ordered {
    ...
    private Mono<Void> verifyParameters(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (currentTime - Long.parseLong(timestamp) >= FIVE_MINUTES) {
            throw new BusinessException(StatusCode.NOT_LOGIN_ERROR, "会话已过期,请重试！");
        }
        try {
            UserVO user = innerUserService.getInvokeUserByAccessKey(accessKey);
            if (user == null) {
                throw new BusinessException(StatusCode.NO_AUTH_ERROR, "请正确配置接口凭证");
            }
            // 校验 accessKey
            if (!user.getAccessKey().equals(accessKey)) {
                throw new BusinessException(StatusCode.NO_AUTH_ERROR, "请先获取请求密钥");
            }
            if (user.getStatus().equals(BAN.getValue())) {
                throw new BusinessException(StatusCode.OPERATION_ERROR, "该账号已封禁");
            }
            // 校验签名
            if (!getSign(body, user.getSecretKey()).equals(sign)) {
                throw new BusinessException(StatusCode.NO_AUTH_ERROR, "非法请求");
            }
            if (user.getBalance() <= 0) {
                throw new BusinessException(StatusCode.OPERATION_ERROR, "余额不足，请先充值。");
            }
            ...
            if (interfaceInfo == null) {
                throw new BusinessException(StatusCode.NOT_FOUND_ERROR, "接口不存在");
            }
            if (interfaceInfo.getStatus() == InterfaceStatusEnum.AUDITING.getCode()) {
                throw new BusinessException(StatusCode.PARAMS_ERROR, "接口审核中");
            }
            if (interfaceInfo.getStatus() == InterfaceStatusEnum.OFFLINE.getCode()) {
                    ...
                    for (RequestParamsField requestParamsField : list) {
                        if ("是".equals(requestParamsField.getRequired())) {
                            if (StringUtils.isBlank((CharSequence) requestBodyMap.get(requestParamsField.getFieldName())) || !requestBodyMap.containsKey(requestParamsField.getFieldName())) {
                                throw new BusinessException(StatusCode.FORBIDDEN_ERROR, "请求参数有误，" + requestParamsField.getFieldName() + "为必选项，详细参数请参考API文档：https://doc.qimuu.icu/");
                            }
                        }
                    }
                }
            ...
            throw new BusinessException(StatusCode.FORBIDDEN_ERROR, e.getMessage());
        }
    }
                        ...
                        return super.writeWith(
                                fluxBody.map(dataBuffer -> {
                                    ...
                                    redissonLockUtil.redissonDistributedLocks(("gateway_" + user.getUserAccount()).intern(), () -> {
                                        ...
                                        if (!invoke) {
                                            throw new BusinessException(StatusCode.OPERATION_ERROR, "接口调用失败");
                                        }
                                    }, "接口调用失败");
